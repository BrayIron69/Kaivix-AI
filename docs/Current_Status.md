...
- ✅ Per-business API keys (Decision #024) — tenant-scoped, storing only a SHA-256 hash and verifying with `secrets.compare_digest`. Plaintext exists once, at generation time
- ✅ Keys live in the `BUSINESS_API_KEYS` environment variable (`auth/business_api_keys.py`), a JSON `{"business_id": "<sha256-of-key>"}` mapping — **not** a local database. The original `auth/api_key_store.py` SQLite store was removed: `auth/api_keys.db` is excluded by both `.gitignore` and `.dockerignore` and Render has no persistent disk, so a key it "issued" existed only on the machine that ran it and production could never hold one. Confirmed against the live deployment before changing anything
- ✅ One variable holding a mapping, rather than a `BUSINESS_API_KEY_<ID>` naming convention: `business_id` is used verbatim everywhere else and can contain characters an env var name cannot (`test-business-b`), and normalising would let `test-business-b` and `test_business_b` collide into the same variable — a silent cross-tenant authentication bug
- ✅ `POST /chat/{business_id}` and `POST /voice/{business_id}/chat/completions` both require a matching `X-API-Key`, enforced as a dependency so it runs **before** config loading and engine construction: an unauthorized caller does no work, and an unknown `business_id` returns the same 401 as a known one rather than acting as an enumeration oracle
...
- ✅ `scripts/generate_api_key.py <business_id>` generates a key and prints the `BUSINESS_API_KEYS` value to set on the host. It **writes nothing** — issuing is now a manual env-var step on Render, the same one already used for `ADMIN_PASSWORD` / `GROQ_API_KEY` / `GOOGLE_CLIENT_SECRET`. Use `--merge-with` to add a business without wiping the others
- ⬜ A key must be set on Render before `/chat/{business_id}` or the Vapi voice endpoint can be used in production — until then both correctly return 401 for every caller
